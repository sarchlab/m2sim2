...
\begin{itemize}
\item A complete cycle-accurate M2 simulator with 14.2\% average timing error across 11 microbenchmarks with hardware CPI comparison
\item Evaluation of 5 PolyBench kernels revealing the in-order vs.\ OoO accuracy gap (120\% average error on intermediate benchmarks)
...
\textbf{PolyBench Suite (5 kernels):} Intermediate-complexity linear algebra kernels including matrix-vector operations (MVT, ATAX, BiCG), matrix multiply (GEMM), and stencil computations (Jacobi-1D). Both simulation and hardware use the SMALL dataset, enabling direct CPI error comparison. These benchmarks reveal the in-order vs.\ OoO accuracy gap.
...
M2Sim achieves 14.2\% average timing error across 11 microbenchmarks, meeting our sub-20\% accuracy target. Including 5 PolyBench kernels with comparable SMALL-dataset hardware CPI, the overall average error across 16 benchmarks is 46.85\%, driven by the in-order vs.\ OoO architectural gap on intermediate benchmarks.

\begin{table}[ht]
\centering
\caption{Timing Accuracy Results (CI-Verified)}
\fontsize{8}{9}\selectfont
\begin{tabular}{lrrr}
\toprule
\textbf{Category} & \textbf{Count} & \textbf{Avg Error} & \textbf{Range} \\
\midrule
Microbenchmarks & 11 & 14.2\% & 1.3\% - 24.6\% \\
PolyBench & 5 & 120.4\% & 66.4\% - 261.8\% \\
\midrule
\textbf{Overall (16 benchmarks)} & \textbf{16} & \textbf{46.85\%} & \textbf{1.3\% - 261.8\%} \\
\bottomrule
\end{tabular}
\label{tab:accuracy}
\end{table}
...
\textbf{In-Order vs. Out-of-Order:} We chose in-order execution for implementation simplicity. While microbenchmarks achieve 14.2\% average error, PolyBench kernels show 120\% average error (66--262\%) because the real M2 is OoO and benefits enormously from instruction-level parallelism on memory-heavy computation patterns. This confirms that the in-order model is the dominant source of error on intermediate benchmarks, not calibration or cache modeling.
...
M2Sim provides the first cycle-accurate simulation infrastructure for Apple M2 CPU research, achieving 14.2\% average timing error across 11 microbenchmarks with hardware CPI comparison. Through systematic hardware baseline methodology and careful microarchitectural modeling, we demonstrate strong timing accuracy on microbenchmarks. Evaluation of 5 PolyBench kernels with comparable SMALL-dataset hardware CPI reveals 120\% average error, identifying the in-order vs.\ OoO architectural gap as the primary accuracy limitation for intermediate benchmarks.
